...
# if content/generated/schema.json does not exist
...
if [ "$GENERATE_SCHEMA" = true ] || [ ! -f $_GRAPHQL_SCHEMA ]; then
  yarn graphql-codegen -r dotenv/config --config content/schema.yml
else
  printf 'Skipping \e[33m`content`\e[m schema generation.\nRun \e[35mGENERATE_SCHEMA=true\e[m \e[34myarn gen\e[m to generate the schema.\nHappy local coding.\n'
fi
